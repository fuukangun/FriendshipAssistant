...
- 左摇杆和方向键可在礼物网格中选择礼物
- 右摇杆左右移动时选择礼物，并将鼠标指针同步到当前礼物中心
- 右摇杆上下移动时滚动礼物网格，不移动鼠标指针
- 打开弹窗或切换背包/存储页后，默认选择当前页面第一个礼物，鼠标指针定位到该礼物中心
- 修复礼物网格四个角落的焦点卡死问题
...
|------|------|
| 左摇杆 / 方向键 | 在礼物网格中选择礼物 |
| 右摇杆左右 | 横向选择礼物，并同步鼠标指针 |
| 右摇杆上下 | 滚动礼物网格，不移动鼠标指针 |
| A | 选择并赠送当前礼物 |
| B | 关闭送礼建议弹窗 |
| X | 记录“今日不再提示”并关闭弹窗 |
| LB / RB | 切换背包页和存储页 |
...
手柄使用游戏原生逻辑按键，不区分 Xbox、PlayStation 或 Switch 控制器品牌，也不会覆盖游戏内自定义按键映射。
...
| 设置 | 默认值 | 说明 |
|------|--------|------|
| 自动交好 | 关闭 | 开启后，对话结束自动从玩家背包中选择并送出最优礼物，不弹出选择界面；不会从存储设备取出物品。 |
